...
function hasAny(text, terms) { const n = normalize(text); return terms.some((term) => n.includes(term)); }
...
function positiveTrigger(text, terms, negations) {
  const n = normalize(text);
  for (const negation of negations || []) if (n.includes(normalize(negation))) return false;
...
function scoreRegistryMatch(requestText, tool) {
  ...
  const text = normalize(requestText); const haystack = normalize([tool.tool_id, tool.name, tool.purpose, tool.input_schema_description, tool.output_schema_description].filter(Boolean).join(' ')); let overlap = 0; for (const token of new Set(tokenize(text))) if (haystack.includes(token)) overlap += 1;
...
async function githubPutFile(path, content) {
  ...
  let sha; const current = await fetch(`${api}?ref=${encodeURIComponent(branch)}`, { headers }); if (current.ok) sha = (await current.json()).sha;
  ...
  const body = await put.json(); return { path, commit: body.commit && body.commit.sha };
}
...
async function foundryOperator(input = {}) {
  ...
  result.results.diagnosis = selfHeal(); result.next_action = result.blockers.length ? 'Resolve blockers before continuing.' : 'Operator completed the requested action.'; return result;
}

const EXECUTABLE_HANDLERS = { idea_analyzer: analyzeIdea, tool_mission_generator: generateMission, foundry_self_healer: selfHeal, foundry_operator: foundryOperator, pdf_tool_mission_planner: planPdfTool, tool_readiness_checker: toolReadinessChecker };
...
module.exports = { STARTED_AT, BUILTIN_TOOL_METADATA, EXECUTABLE_HANDLERS, getTools, executeTool, registerTool, createMission, getMissionStatus, evaluateTool, selfHeal, toolReadinessChecker };
