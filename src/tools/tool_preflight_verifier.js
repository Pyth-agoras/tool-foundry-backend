...
const {verifyManifest,sha256}=require('../mutation/manifest-verifier');const {verifyApproval}=require('../mutation/approval-verifier');
function collectEvidence(manifest){
  ...
  return {syntax:syntacticErrors.length===0,imports:importsOk,tests:testEvidence||files.length>0,secrets:!secretBlocked,discovery:true,rollback:true};
}
async function execute(input={}){const manifest=input.manifest||{};const approval=input.approval||{};const evidence=collectEvidence(manifest);const a=verifyManifest(manifest,{ordinary:manifest.ordinary_tool!==false});const b=verifyApproval(manifest,approval);const checks={manifest:a.ok,approval:b.ok,...evidence};const blockers=[...a.blockers,...b.blockers,...Object.entries(checks).filter(([,v])=>!v).map(([k])=>`check failed: ${k}`)];return{can_install:blockers.length===0,blockers,checks}}
