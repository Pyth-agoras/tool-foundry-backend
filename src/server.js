...
require('./backend_source_inspector').install(router);
...
app.post('/tools/register', (req, res) => {
  try {
    ok(res, router.registerTool(req.body || {}));
  } catch (error) {
    fail(res, error.statusCode || 500, error.message || 'Tool registration failed.');
  }
});

app.post('/tools/mission/create', (req, res) => {
  try {
    ok(res, router.createMission(req.body || {}));
  } catch (error) {
    fail(res, error.statusCode || 500, error.message || 'Mission creation failed.');
  }
});

app.get('/tools/mission/:id/status', (req, res) => {
  try {
    ok(res, router.getMissionStatus(req.params.id));
  } catch (error) {
    fail(res, error.statusCode || 500, error.message || 'Mission lookup failed.');
  }
});
...
app.post('/tools/mission/:id/revision', (req, res) => {
  try {
    ...
    fail(res, error.statusCode || 500, error.message || 'Revision failed.');
  }
});

app.post('/tools/evaluate', (req, res) => {
  try {
    ok(res, router.evaluateTool(req.body || {}));
  } catch (error) {
    fail(res, error.statusCode || 500, error.message || 'Evaluation failed.');
  }
});
