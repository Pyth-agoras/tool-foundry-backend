'use strict';
const test=require('node:test'),assert=require('node:assert'),fs=require('fs'),os=require('os'),path=require('path'),http=require('http');
...
function makeManifest(overrides={}){const c='const ok=true;';return {repository:'o/r',base_commit:'56ab494159cd0f877aa2c666639f1cba8dc8753a',target_branch:'b',default_branch:'main',ordinary_tool:true,files:[{path:'src/tools/x.js',content:c,before_sha256:sha256(c),after_sha256:sha256(c),full_returned:true,truncated:false,redacted:false}],protected_effects:[],...overrides}};
function makeApproval(manifest){const secret=process.env.APPROVAL_SIGNING_SECRET || 'test-secret';return {repository:manifest.repository,target_branch:manifest.target_branch,paths:manifest.files.map(f=>f.path),protected_effects:manifest.protected_effects||[],receipt:signReceipt({repository:manifest.repository,base_commit:manifest.base_commit,target_branch:manifest.target_branch,paths:manifest.files.map(f=>f.path),before_hashes:manifest.files.map(f=>({path:f.path,before_sha256:f.before_sha256||''})),after_hashes:manifest.files.map(f=>({path:f.path,after_sha256:f.after_sha256||''})),protected_effects:manifest.protected_effects||[],expires_at:'',transaction_id:''},secret)}};
...
test('health flags reflect runtime auth and gate availability',async()=>{process.env.API_KEY='k';process.env.APPROVAL_SIGNING_SECRET='test-secret';config.apiKey='k';const s=server.createServer().listen(0);try{const res=await request(s,'/health');assert.strictEqual(res.body.auth_enforced,true);assert.strictEqual(res.body.mutation_gates_enforced,true)}finally{await new Promise(r=>s.close(r))}})
